...
void adc_reg_t1(ubit16_t opcode, machine_t& machine) {
    ...
    ubit32_t shifted = shift<ubit32_t, 32>(GET_R(rmx, machine.arm), lsl, 0, G_CF(machine.arm) ? 1 : 0);
...
void add_reg_t1(ubit16_t opcode, machine_t& machine) {
    ...
    ubit32_t shifted = shift<ubit32_t, 32>(GET_R(mx, machine.arm), lsl, 0, G_CF(machine.arm) ? 1 : 0);
    triad<ubit32_t, ubit1_t, ubit1_t> result = add_with_carry<ubit32_t, 32>(GET_R(nx, machine.arm), shifted, 0);
...
void add_reg_t2(ubit16_t opcode, machine_t& machine) {
    ...
    ubit32_t shifted = shift<ubit32_t, 32>(GET_R(mx, machine.arm), lsl, 0, G_CF(machine.arm) ? 1 : 0);
    triad<ubit32_t, ubit1_t, ubit1_t> result = add_with_carry<ubit32_t, 32>(GET_R(dnx, machine.arm), shifted, 0);
...
void add_reg_sp_t1(ubit16_t opcode, machine_t& machine) {
...
void add_reg_sp_t2(ubit16_t opcode, machine_t& machine) {
    ubit32_t& sp = GET_R(13, machine.arm);
    sp = add_with_carry<ubit32_t, 32>(sp, mask_bits<ubit32_t, ubit16_t, 0, 7>(opcode) << 2, 0).item1;
}

void add_reg_sp_t3(ubit16_t opcode_h, ubit16_t opcode_l, machine_t& machine) {
    ubit4_t dx = mask_bits<ubit4_t, ubit16_t, 8, 4>(opcode_l);
    bool set_flag = mask_bit<ubit1_t, ubit16_t, 4>(opcode_h) == 1;
